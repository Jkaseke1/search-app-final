...
const MainComponent = ({ favouritesProp, setFavouritesProp }) => {
    ...
    const modifyTypedInput = () => {
        return searchStore.typedInput.split('').map(char => char === ' ' ? '+' : char).join('');
    };
    ...
    const searchFunc = async (filterBtn = false) => {
        if (searchStore.typedInput) {
            ...
            try {
                ...
                if (!filterBtn) {
                    res = await fetch(`https://itunes.apple.com/search?term=${modifyTypedInput()}${searchStore.clickedFilter}`);
                } else {
                    res = await fetch(`https://itunes.apple.com/search?term=${modifyTypedInput()}&media=${filterBtn}`);
                }
    ...
    const likeUnlikeFunc = (dataIndex) => {
        ...
        const favouriteIndex = favouritesProp.findIndex(favourite => favourite === apiData[dataIndex]);

        if (favouriteIndex === -1) { // Favourite not found, add it
            setFavouritesProp([...favouritesProp, apiData[dataIndex]]);
            apiData[dataIndex].likeStatus = true;
        } else { // Favourite found, remove it
    ...
    return (
        <main className="container fluid text-center">
            ...
            <hr />
            <ResultsComponent apiDataProp={apiData} likeUnlikeFuncProp={likeUnlikeFunc} />
        </main>
    );
};
